...
- 每日状态按账号、年份和源记录身份保存在私有 Store；已覆盖压力摘要、HRV、睡眠摘要、
  `sleepNeed`、`nextSleepNeed`、健身年龄、训练状态、VO2 Max 和训练负荷重点。
- 每日状态中的数值字段以 Garmin 源日期或源时间投影为长期 Recorder statistics；Store 是
  规范记录，statistics 是可重建的查询投影。
...
每日状态 Store 和 Recorder 投影已在 `3.1.0-beta.14` 现场验证。第 8 节记录其当前数据身份、
来源日期和投影行为；它不是待实现设计。
...
| beta.14 已归档 | 当前插件已经归一化并持久化，且完成现场读取验证 |
...
2026-08-01 `3.1.0-beta.14` 现场只读盘点：
...
`3.1.0-beta.14` 已实现本节的年度私有 Store 和数值 Recorder 投影。
...
- Store 按账号和年份隔离。记录身份由数据族、源记录 key、Garmin `calendarDate` 和可选
  设备来源组成；设备 ID 只保存不可逆短哈希，不暴露为健康值。
- `sleepNeed` 和 `nextSleepNeed` 是独立记录；训练状态和训练负荷按设备拆分；通用 VO2 Max
  使用 `generic` 来源，不因缺少设备 ID 被丢弃。
- 请求日期不等于记录日期。每条记录优先使用子来源自己的 `calendarDate`：通用 VO2 Max
  可能属于更早日期，`nextSleepNeed` 可能属于次日。缺少明确源时刻时，才使用 UTC+8 的
  来源日历日起点作为 statistic 时间桶。
- 同一天相同内容幂等跳过；Garmin 后续返回修订值时更新该日记录；临时空响应不能删除
  已有正常数据。
- 写入采用 Store-first copy-on-write：新 revision 先持久化到 Store，成功后才更新内存并
  投影 Recorder；Store 保存失败不会泄漏未持久化 revision。Recorder 投影失败时保留未投影
  revision，后续归档会重试，成功后再持久化 `projected_revision`。
...
- 当前不开放每日状态专用 REST、WebSocket、响应动作或自定义卡片。外部分析仍以规范归档
  为数据基础，查询表面留待数据稳定积累后单独设计。
...
当前训练归档将每台设备的训练状态、急慢性负荷和 ACWR 保存为独立源记录；训练负荷重点
也按设备拆分，目标范围和反馈保存在 Store，数值使用带设备来源后缀的 statistic ID。
VO2 Max 按返回的子来源拆分；没有设备 ID 的通用值使用 `generic` 身份，并以该子来源自己的
`calendarDate` 落库。不同来源不会互相覆盖。
...
`sleep_summary`、`sleep_sleep_need` 和 `sleep_next_sleep_need` 是三个独立源记录。两份睡眠
需求各自保存实际值、baseline、调整、反馈、推荐时间、设备来源和字段 presence；数值字段
分别投影长期 statistics。`nextSleepNeed` 的来源日期可以晚于睡眠摘要日期，不能按请求日
把它并入或覆盖 `sleepNeed`。
...
当前实现已将健身年龄结果及已确认的组成数值写入年度状态 Store，并将数值投影为长期
statistics。缺失组成项继续记录字段 presence，不用最近实体值填补。
...
替代。当前实现已按源 `calendarDate` 保存压力摘要记录，并把可用数值投影为长期
statistics；`stressQualifier` 等枚举只保存在结构化记录和最新状态表面，不伪装成数值。

### 8.7 当前设备确认不支持的数据

下列能力在当前账号接口中返回空值，并已在 Garmin App 确认不可见。它们是当前设备能力
限制，不是解析或持久化缺陷。兼容实体保留且默认禁用；正常 TrainingCoordinator 不自动
请求对应端点，避免反复调用已知为空的设备能力：

| 数据族 | 当前结论 | 处理 |
| --- | --- | --- |
| 训练准备度 | 接口为空，App 无 | 兼容实体默认禁用；端点不自动轮询；不建立新归档记录 |
...
- 每日状态 Store 当前只覆盖已确认 schema；新增或变化字段仍需通过捕获、重放和显式 schema
  扩展，不能静默猜测语义。
- 体重和体成分、血压、饮水、Move IQ、异常心率等测量或事件族尚未统一为长期事件模型。
- 活动会话摘要、Calendar 与 FIT 会话内传感器流之间的规范查询关系仍待设计。

## 11. 下一轮讨论

跟踪级曲线和每日状态快照的模型已经落地。下一轮继续讨论尚未完成数据身份设计的离散
测量、事件和会话：
