# Garmin Connect 3.1.0-beta.13

This release adds source-date daily status storage for prospective Garmin
history. HRV, training status and workload, sleep scores and sleep need,
fitness age, and daily stress summaries are checkpointed in private annual
...
Garmin request. Empty later responses cannot erase a valid daily record.

Current-value entities remain useful as latest-value views. Daily snapshot
entities no longer claim sync-time long-term statistics where source-date
statistics are now canonical, and large training/HRV attributes are bounded.
...
Archive cadence and existing UTC+8 source-day fallback remain unchanged.

Upgrade to `3.1.0-beta.13`, then restart Home Assistant from its control panel.
Enable Prospective Archive if it is not already enabled. After Garmin has
synced and one archive cycle has completed, verify that History Status returns
to `idle`, supported current entities retain values, and Recorder statistics
whose IDs begin with `garmin_connect:` contain source-date HRV, sleep, stress,
fitness-age, and training points.

Daily status records are retained in files named
